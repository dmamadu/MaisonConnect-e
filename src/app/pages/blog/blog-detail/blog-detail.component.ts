...
export class BlogDetailComponent implements OnInit, OnDestroy {
  ...
  loadPost(id: number): void {
    ...
    this.baseService
      .getBlogById(id)       // GET /api/public/posts/{id}
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response: any) => {
          this.loadData = false;
        ...
        error: (err) => {
          ...
          // this.snackbar.error('Article introuvable.');
        }
      });
  }

  // ─── Contenu HTML sécurisé ───────────────────────────────────────────────────
  private buildSafeContent(post: BlogPost): void {
    const raw = post.content
      ? (post.content[this.lang] ?? post.content['fr'] ?? '')
      : (post.excerpt[this.lang] ?? post.excerpt['fr'] ?? '');
  ...
  private loadRelatedPosts(current: BlogPost): void {
    this.baseService
      ...
      .subscribe({
        next: (response: any) => {
          const all: BlogPost[] = Array.isArray(response)
            ? response
            : (response?.data ?? []);

          // Même catégorie, pas le post actuel
          const same = all.filter(
            p => p.category?.id === current.category_id && p.id !== current.id
          );

          // Compléter si moins de 3
          const others = all.filter(
            p => p.id !== current.id && !same.find(s => s.id === p.id)
          );

          this.relatedPosts = [...same, ...others].slice(0, 3);
        },
  ...
  getTitle(post: BlogPost): string {
    return post.title[this.lang] ?? post.title['fr'];
  }

  getExcerpt(post: BlogPost): string {
    return post.excerpt[this.lang] ?? post.excerpt['fr'];
  }
