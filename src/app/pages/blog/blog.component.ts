...
export interface BlogPost {
  ...
  image: string;           // chemin relatif ex: "blog/xxx.png"
  author: string;
  read_time: string;
  published_at: string;
  created_at: string;
...
export class BlogComponent implements OnInit, OnDestroy {
  ...
  getBlog(): void {
    ...
    this.baseService
      .all('posts')          // GET /api/public/posts
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response: any) => {
          this.loadData = false;
          // L'API peut retourner directement un tableau OU { data: [...] }
          const data: BlogPost[] = Array.isArray(response)
            ? response
            : (response?.data ?? []);
        ...
        error: (err) => {
          ...
          // this.snackbar.error('Impossible de charger les articles.');
        }
      });
  }

  // ─── Construire les filtres catégories depuis les posts ────────────────────
  private buildCategories(posts: BlogPost[]): void {
    const seen = new Set<number>();
    const extras: { value: string; label: string }[] = [];

    for (const post of posts) {
      if (post.category && !seen.has(post.category.id)) {
        seen.add(post.category.id);
        extras.push({
          value: post.category.slug,
          label: this.getCategoryName(post.category)
        });
      }
    }
  ...
  getTitle(post: BlogPost): string {
    ...
    return post.title[lang as 'fr' | 'en'] ?? post.title['fr'];
  }

  /** Retourne l'extrait dans la langue active */
  getExcerpt(post: BlogPost): string {
    const lang = this.translate.currentLang || 'fr';
    return post.excerpt[lang as 'fr' | 'en'] ?? post.excerpt['fr'];
  }

  /** Construit l'URL complète de l'image */
  getImageUrl(path: string): string {
    if (!path) return 'assets/images/placeholder-blog.png';
    if (path.startsWith('http')) return path;
    return STORAGE_BASE + path;
  }

  /** Nom de catégorie dans la langue active */
  getCategoryName(category: BlogCategory): string {
    const lang = this.translate.currentLang || 'fr';
    return category?.name?.[lang as 'fr' | 'en'] ?? category?.name?.['fr'] ?? '';
  }

  /** Fallback image cassée */
  onImageError(event: Event): void {
    (event.target as HTMLImageElement).src = 'assets/images/placeholder-blog.png';
  }
  ...
  /** Posts filtrés selon la catégorie sélectionnée */
  get filteredPosts(): BlogPost[] {
    if (this.selectedCategory() === 'all') return this.posts;
    return this.posts.filter(p => p.category?.slug === this.selectedCategory());
  }
}